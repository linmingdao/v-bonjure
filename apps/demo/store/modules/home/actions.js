...
export default function(logger) {
    return {
        ...
        async [ACTIONS.GET_TODOLIST]({ dispatch, commit, getters, rootGetters, rootState }, payload) {
            // Step_1: 将全局的pending标志位置为true，应用根据该标志显示loading效果
            commit(`${GLOBAL.MUTATIONS.ENABLE_PENDING}`, null, { root: true });

            // Step_2: Http网络请求
            const response = await getTodolist(payload);
            // 非异步的形式，返回的是 Promise ，则需要嵌套写法，可能会出现回调地狱
            // getTodolist(payload).then(response => {
